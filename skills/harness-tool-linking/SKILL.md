...
description: Detect which AI tool the user runs and write the matching ai-harness adapter file(s). Claude Code, Codex, Pi, GitHub Copilot CLI, and Google Antigravity CLI are fully automated; other tools are recognized and given a manual snippet. Used by /sth-harness:init and the Codex/Pi/Copilot CLI/AGY sth-harness skill.
---
...
- If running from GitHub Copilot CLI, propose **GitHub Copilot CLI** and ask the
  user to confirm or pick another.
- If running from Google Antigravity CLI (`agy`), propose **Google Antigravity
  CLI** and ask the user to confirm or pick another.
- If there is no signal, ask the user to choose from: Claude Code, Codex,
  Google Antigravity CLI, GitHub Copilot CLI, GitHub Copilot IDE, Gemini CLI,
  Pi, Aider, Other.
...
### GitHub Copilot CLI (fully automated)
Run the bundled wiring script:
...
### Google Antigravity CLI / AGY (fully automated)
Run the bundled wiring script:
...
### GitHub Copilot IDE / Gemini CLI / Aider (recognized, not yet automated)
...
## 4. Report
State which adapter file(s) were created/appended/skipped, or which snippet was
printed for manual setup.
