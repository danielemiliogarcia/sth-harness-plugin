---
name: sth-harness
description: Codex, Pi, Copilot CLI, and AGY entry point for installing ai-harness, adding feature specs, and implementing the next pending spec without Claude slash commands.
---
...
- In Claude Code, `CLAUDE_PLUGIN_ROOT` may also point at the plugin root, but
  do not require it in Codex, Pi, Copilot CLI, or AGY.
...
5. Before finishing, re-read every file this init flow created or edited
   ...
   decision records.
6. Report what was created and tell the user they can ask Codex, Pi, Copilot
   CLI, or AGY to implement the next pending spec when ready.
