...
check "$(jq -r '.keywords | index("antigravity") != null' "$ROOT/plugin.json" 2>/dev/null)" "true" "antigravity keyword"
check "$(jq -r '.keywords | index("agy") != null' "$ROOT/plugin.json" 2>/dev/null)" "true" "agy keyword"
...
for sc in copy-harness link-claude-code link-codex link-pi link-copilot-cli link-agy seed-readme; do
